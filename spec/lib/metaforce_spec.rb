...
describe Metaforce do
  ...
  describe '#login' do
    let(:args) { {:username => 'foo', :password => 'foobar', :security_token => 'whizbang', :host => 'quux'} }

    context 'when environment variables are not defined' do
      it 'proxies the login call' do
        expect(Metaforce::Login).to receive(:new).with('foo', 'foobar', 'whizbang', 'quux').and_call_original
