...
describe Metaforce::Job do
  ...
  describe '.on_complete' do
    it 'allows the user to register an on_complete callback' do
      expect(job).to receive(:status).at_least(1).times.and_return(Hashie::Mash.new(done: true, state: 'Completed'))
      called = false
  ...
  describe '.on_error' do
    it 'allows the user to register an on_error callback' do
      expect(job).to receive(:status).at_least(1).times.and_return(Hashie::Mash.new(done: true, state: 'Error'))
      called = false
      block = lambda { |job| called = true }
      job.on_error &block
      job.perform
  ...
  describe '.done?' do
    ...
    context 'when done' do
      before do
        expect(job).to receive(:status).and_return(Hashie::Mash.new(done: true))
      end
    ...
    context 'when not done' do
      before do
        expect(job).to receive(:status).and_return(Hashie::Mash.new(done: false))
      end
  ...
  describe '.state' do
    ...
    context 'when done' do
      before do
        expect(job).to receive(:status).and_return(Hashie::Mash.new(done: true, state: 'Completed'))
      end
    ...
    context 'when not done' do
      before do
        expect(job).to receive(:status).once.and_return(Hashie::Mash.new(done: false))
      end
  ...
  %w[Queued InProgress Completed Error].each do |state|
    describe ".#{state.underscore}?" do
      before do
        expect(job).to receive(:status).and_return(Hashie::Mash.new(done: true, state: state))
      end
